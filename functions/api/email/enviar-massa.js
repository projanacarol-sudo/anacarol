...
export async function onRequestPost(context) {
  ...
  const { assunto, html, offset = 0, postId = null } = body;
  ...
  const q = `/leads?select=id,email&opt_in_email=eq.true&unsubscribed_email=eq.false&email_normalizado=not.is.null&order=id&limit=100&offset=${offset}`;
  let leads = [];
  try {
